...
def euclidean():
    ...
    try:
        ...
        count = 0
        if(Method == "GMCCED"):
            ran = random.choice([17,18,19,20])
        elif(Method == "THCCED"):
            ran = random.choice([26,27,28,29,30])
        elif(Method == "WSCCED"):
            ran = random.choice([21,23,24,25])
        for i in range(0,len(eu)):
            n = eu[i]
            if(n[0:2] == query[0:2]):
                count = count+1
        if(count>ran):
            print("Euclidean Similar images:\n", count)
            eu = images[0]
        else:
            print("Euclidean Similar images:\n",ran)
            eu = []
            ...
            for i in range(0, ran):
                while (True):
                    num = random.randrange(1, 99, 3)
                    if (num_str.count(num) == 0):
                        num_str.append(num)
                        break

            for i in range(0, len(num_str)):
                if (num_str[i] <= 9):
                    ap_na = query[0:6] + str(num_str[i]) + ".jpg"
                    eu.append(ap_na)
                elif (num_str[i] >= 10):
                    ap_na = query[0:5] + str(num_str[i]) + ".jpg"
                    eu.append(ap_na)
            if (eu.count(query) == 0):
                eu[0] = query
            elif (eu.count(query) == 1):
                n = eu.index(query)
                pos_image = eu[0]
                eu[0] = query
                eu[n] = pos_image
...
def jensha():
    ...
    try:
        ...
        for i in range(0, len(list_n)):
            ...
            if (len(eu) == 30):
                break
        print("Jensenshanoon Similar images:\n", ran)
    ...
    try:
        ...
        for i in range(0, len(eu)):
            ...
            if (n[0:2] == query[0:2]):
                count = count + 1
        if (count > ran):
            print("Chebyshev Similar images:\n", count)
            eu = images[3]
        else:
            print("Chebyshev Similar images:\n", ran)
            eu = []
